<template>
  <div class="record-body">
        ...
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-if="index === 2">
            ...
            <Checkbox
              ...
              v-else-if="column.dataIndex === 'status' && text"
              >{{ text }}</Checkbox
            >
            <div
    ...
    <div class="flex items-center mt-30px justify-end">
      <span class="text-[#2D3A2F] text-xs">助理签字区</span>
      <div
...
const data = ref([
  ...
  [
    {
      name: '体重',
      matter: '使用体重秤测试，记录以千克为单位，保留小数点后1位',
    },
    {
      name: '身高',
      matter: '使用身高计测试，记录以厘米为单位，保留小数点后1位',
    },
    {
      name: '身高标准体重',
      matter:
        '不同的身高段，对应不同的体重区间，体重区间分为正常、偏瘦、瘦、 超重、胖五个等级',
    },
    {
      name: '双脚连续跳',
      status: '无法测试',
      ...
      test2: 's',
      matter:
        '测试两次，取快值。以秒为单位，保留小数点后一位，小数点后第二位 按"非零进一"原则进位',
    },
    {
      name: '坐位体前屈',
      status: '无法测试',
      test1: 'cm',
      test2: 'cm',
      matter: '测试两次，取最大值。记录以厘米为单位，保留小数点后1位',
    },
